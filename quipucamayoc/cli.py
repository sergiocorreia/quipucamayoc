...
def extract_tables(filename, directory, extension, keep, engine, output, page_append):
    if engine == 'aws':
        ...
        aws_extract_tables(filename=filename, directory=directory, extension=extension, keep_in_s3=keep, ignore_cache=False, output=output, page_append=page_append)
